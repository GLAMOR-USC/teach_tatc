import torch
from .encodings import PosEncoding, PosLearnedEncoding, TokenLearnedEncoding
...
class EncoderVL(nn.Module):
    ...
    def forward(
        ...
        mask_pad = torch.zeros((len(emb_lang), length_mask_pad),
        ...
        for i, (len_l, len_f, len_a) in enumerate(
            ...
            # mask padded frames
            mask_pad[i, length_lang_max + len_f:length_lang_max +
                     length_frames_max] = True
            # mask padded actions
            mask_pad[i, length_lang_max + length_frames_max + len_a:] = True

        # encode the inputs
        emb_all = self.encode_inputs(emb_lang, emb_frames, emb_actions,
        ...
        # encode the inputs
        output = self.enc_transformer(emb_all.transpose(0, 1), mask_attn,
                                      mask_pad).transpose(0, 1)
        return output, mask_pad

    def encode_inputs(self, emb_lang, emb_frames, emb_actions, lengths_lang,
                      lengths_frames, mask_pad):
        """
