...
class PosLearnedEncoding(nn.Module):
    ...
    def forward(self, lang, frames, actions, lens_lang, lens_frames):
        pos_lang = torch.stack([torch.arange(0, lang.shape[1])] *
                               lang.shape[0])
...
class TokenLearnedEncoding(nn.Module):
    ...
    def forward(self, lang, frames, actions):
        token_lang = torch.ones(
            lang.shape[:2], device=lang.device, dtype=torch.long) * 0
        token_lang_emb = self.emb(token_lang)
        lang += token_lang_emb
        token_frames = torch.ones(
            frames.shape[:2], device=frames.device, dtype=torch.long) * 1
        token_frames_emb = self.emb(token_frames)
        frames += token_frames_emb
        token_actions = torch.ones(
            actions.shape[:2], device=actions.device, dtype=torch.long) * 2
...
class DatasetLearnedEncoding(nn.Module):
    ...
    def forward(self, lang, vocab):
        dataset_ids = torch.ones(lang.shape[0],
                                 device=lang.device,
                                 dtype=torch.long)
