...
class Definitions:
    def __init__(self, definitions=None, simulator="THOR", version="2.0"):
        ...
        if definitions is None:
            with importlib.resources.open_text(
                    meta_data_files, "default_definitions.json") as data_file:
                definitions = json.load(
                    data_file, object_pairs_hook=OrderedDict)["definitions"]

            if version == "2.0" and simulator == "THOR":
                tasks, task_id_to_task_dict, task_name_to_task_dict = Task_THOR.load_tasks(
                    task_definitions)
                definitions["tasks"] = tasks
                self.map_tasks_id2info = task_id_to_task_dict
                self.map_tasks_name2info = task_name_to_task_dict
            else:
                raise RuntimeError("No support for version " + str(version) +
                                   " with simulator " + str(simulator))
    ...
    def to_dict(self):
        ...
        info_dict["tasks"] = [
            x.to_dict() if not isinstance(x, dict) else x
            for x in info_dict["tasks"]
        ]
        return info_dict
