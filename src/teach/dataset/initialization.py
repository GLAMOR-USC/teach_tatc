...
class Initialization:
    ...
    def from_dict(cls, initialization_dict) -> "Initialization":
        ...
        if "agents" in initialization_dict:
            agents = [
                Pose_With_ID.from_dict(x)
                for x in initialization_dict["agents"]
            ]

        if "objects" in initialization_dict:
            objects = [
                Pose_With_ID.from_dict(x)
                for x in initialization_dict["objects"]
            ]

        return cls(time_start=initialization_dict["time_start"],
                   agents=agents,
                   objects=objects)
