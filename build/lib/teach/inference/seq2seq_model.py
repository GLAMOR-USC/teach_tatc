...
from modeling.datasets import TATCDataset
...
class Seq2SeqModel(TeachModel):
    ...
    def __init__(self, process_index: int, num_processes: int,
        ...
        parser.add_argument("--model_dir",
                            type=str,
                            required=True,
                            help="Model folder name under $TEACH_LOGS")
    ...
    def start_new_tatc_instance(self,
        ...
        self.cur_tatc_instance = data_util.process_traj(
            tatc_instance,
            Path(os.path.join("test", tatc_instance["instance_id"])), 0,
            self.preprocessor)
        feat_numpy = {
            "lang": GuidestatcDataset.load_lang(self.cur_tatc_instance)
        }
        _, self.input_dict, _ = data_util.tensorize_and_pad(
            [(self.cur_tatc_instance, feat_numpy)], self.args.device,
            constants.PAD)

        if not self.args.skip_tatc_history and tatc_history_images is not None and len(
                tatc_history_images) > 0:
            img_features = self.extractor.featurize(tatc_history_images,
                                                    batch=32)
