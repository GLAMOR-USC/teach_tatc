...
class ETModel(TeachModel):
    ...
    def __init__(self, process_index: int, num_processes: int,
        ...
        parser.add_argument("--model_dir",
                            type=str,
                            required=True,
                            help="Model folder name under $ET_LOGS")
        parser.add_argument("--checkpoint",
                            type=str,
                            default="latest.pth",
                            help="latest.pth or model_**.pth")
        parser.add_argument("--object_predictor",
                            type=str,
                            required=True,
                            help="Path to MaskRCNN model checkpoint")
        ...
        parser.add_argument(
            ...
            default=False,
            help=
            "Specify this to ignore actions and image frames in EDH history",
        )
    ...
    def set_up_model(self, process_index):
        ...
        dataset_info = data_util.read_dataset_info_for_inference(
            self.args.model_dir)
        train_data_name = self.et_model_args.data["train"][0]
        train_vocab = data_util.load_vocab_for_inference(
            self.args.model_dir, train_data_name)
        ...
        if model_path is not None:
            ...
            self.model, self.extractor = eval_util.load_agent(
                model_path, dataset_info, self.args, for_inference=True)

        self.vocab = {
            "word": train_vocab["word"],
            "action_low": self.model.vocab_out
        }
    ...
    def start_new_edh_instance(self,
        ...
        self.cur_edh_instance = data_util.process_traj(
            edh_instance,
            Path(os.path.join("test", edh_instance["instance_id"])), 0,
            self.preprocessor)
        feat_numpy = {
            "lang": GuidesEdhDataset.load_lang(self.cur_edh_instance)
        }
        _, self.input_dict, _ = data_util.tensorize_and_pad(
            [(self.cur_edh_instance, feat_numpy)], self.args.device,
            constants.PAD)

        if not self.args.skip_edh_history and edh_history_images is not None and len(
                edh_history_images) > 0:
            img_features = self.extractor.featurize(edh_history_images,
                                                    batch=32)
            self.model.frames_traj = img_features
            self.model.frames_traj = torch.unsqueeze(self.model.frames_traj,
                                                     dim=0)
    ...
    def get_next_action(self,
        ...
        with torch.no_grad():
            ...
            m_out = self.model.step(self.input_dict,
                                    self.vocab,
                                    prev_action=prev_api_action)

        m_pred = model_util.extract_action_preds(m_out,
                                                 self.model.pad,
                                                 self.vocab["action_low"],
                                                 clean_special_tokens=False)[0]
        action = m_pred["action"]

        obj = None
        if action in obj_interaction_actions and len(
                m_pred["object"]) > 0 and len(m_pred["object"][0]) > 0:
            obj = m_pred["object"][0][0]

        predicted_click = None
        if obj is not None:
            predicted_click = self.get_obj_click(obj, img)
        logger.debug("Predicted action: %s, obj = %s, click = %s" %
                     (str(action), str(obj), str(predicted_click)))
