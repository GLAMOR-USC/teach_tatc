...
class InferenceRunner:
        ...
        try:
            for process_index in range(config.num_processes):
                ...
                process = InferenceRunner._launch_process(
                    process_index, edh_instance_files, config, er)
    ...
    @staticmethod
    def _launch_process(process_index, edh_instance_files,
        ...
        process = mp.Process(target=InferenceRunner._run,
                             args=(process_index, files_to_process, config,
                                   er))
    ...
    def _run(process_index, files_to_process, config: InferenceRunnerConfig,
        ...
        for file_index, instance_file in enumerate(files_to_process):
            try:
                instance_id, instance_metrics = InferenceRunner._run_edh_instance(
    ...
    def _load_edh_history_images(edh_instance, config: InferenceRunnerConfig):
        ...
        image_dir = os.path.join(config.data_dir, "images", config.split,
                                 edh_instance["game_id"])
        return load_images(image_dir, image_file_names)

    @staticmethod
    def _run_edh_instance(instance_file, config: InferenceRunnerConfig,
                          model: TeachModel, er: EpisodeReplay):
        edh_instance = InferenceRunner._load_edh_instance(instance_file)
        edh_instance["state_changes"] = get_state_changes(
            edh_instance["tasks"][0]["episodes"][0]['initial_state'],
            edh_instance["tasks"][0]["episodes"][0]["final_state"])
        ...
        try:
            init_success, er = with_retry(
                fn=lambda: InferenceRunner._initialize_episode_replay(
                    edh_instance, game_file, edh_check_task, config.
                    replay_timeout, er),
        ...
        try:
            ...
            init_success = False
            logger.error(
                f"Failed to load_edh_history_images for {instance_id}",
                exc_info=True)
        ...
        try:
            ...
            model_started_success = False
            metrics["error"] = 1
            logger.error(f"Failed to start_new_edh_instance for {instance_id}",
                         exc_info=True)
            ...
            for _ in range(config.max_traj_steps):
                ...
                try:
                    ...
                    image_name = InferenceRunner._save_image(
                        config, edh_instance, img, traj_steps_taken)
                    action, obj_relative_coord = model.get_next_action(
                        img, edh_instance, prev_action, image_name,
                        instance_file)
                    step_success = InferenceRunner._execute_action(
                        er.simulator, action, obj_relative_coord)
                    InferenceRunner._update_metrics(metrics, action,
                                                    obj_relative_coord,
                                                    step_success)
                    prev_action = {
                        "action": action,
    ...
    def _initialize_episode_replay(edh_instance, game_file, task,
                                   replay_timeout, er: EpisodeReplay):
        start_time = time.perf_counter()
        er.set_episode_by_fn_and_idx(game_file, 0, 0)
        edh_interactions = list()
        interactions = edh_instance["tasks"][0]["episodes"][0][
            "interactions"]  #[: edh_instance["pred_start_idx"]]
