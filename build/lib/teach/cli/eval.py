...
def main():
    ...
    edh_instance_files = set(
        os.listdir(os.path.join(args.data_dir, "edh_instances", args.split)))
    output_files = [
        ...
        if not os.path.isfile(pred_actions_file):
            logger.warning(
                ...
                % (os.path.basename(output_file).split("__")[1], output_file,
                   pred_actions_file))
            edh_instance_files_missing_output.append(
