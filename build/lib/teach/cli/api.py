...
def parse_args():
    ...
    arg_parser.add_argument(
        ...
        required=True,
        help=
        'Base data directory containing subfolders "games" and "edh_instances',
    )
    arg_parser.add_argument(
        "--images_dir",
        type=str,
    ...
    arg_parser.add_argument("--use_edh_file",
                            dest="use_edh_file",
                            action="store_true",
                            help="Use edh file instead of request json.")
...
def _get_edh_instance(req_args):
    if teach_args.use_edh_file:
        ...
        edh_instance_path = os.path.join(teach_args.data_dir, "edh_instances",
                                         teach_args.split, req_args.edh_name)
...
def _get_edh_history_images(edh_name, edh_instance):
    ...
    try:
        ...
        if not edh_history_images:
            image_dir = os.path.join(teach_args.data_dir, "images",
                                     teach_args.split, edh_instance["game_id"])
...
def start_new_edh_instance():
    ...
    app.logger.info(
        f"start_new_edh_instance with edh_name={req_args.edh_name}")
    edh_instance, err_msg = _get_edh_instance(req_args)
    if err_msg:
        return err_msg, 500
    edh_history_images, err_msg = _get_edh_history_images(
        req_args.edh_name, edh_instance)
    if err_msg:
        return err_msg, 500
    try:
