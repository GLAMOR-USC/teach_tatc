...
class Task_THOR(Task):
    def __init__(
        ...
        super().__init__(task_id, task_name, task_nparams, task_params,
                         subgoals, comments, episodes)
    ...
    def component_to_dict(component):
        if "task_name" not in component:
            ...
            component_dict["task"] = (component["task"].to_dict() if
                                      not isinstance(component["task"], dict)
                                      else component["task"])
    ...
    def from_dict(cls,
                  task_dict,
                  definitions,
                  process_init_state=True) -> "Task_THOR":
        episodes = [
    ...
    def load_tasks(resource_package):
        ...
        for task_file in importlib.resources.contents(resource_package):
            if not importlib.resources.is_resource(resource_package,
                                                   task_file):
                continue
            ...
            with importlib.resources.open_text(resource_package,
                ...
                if task.task_id in task_id_to_task_dict.keys():
                    raise RuntimeError("Duplicate task_id " +
                                       str(task.task_id) +
                                       " with one occurrence in " +
                                       str(task_file))
                if task.task_name in task_name_to_task_dict.keys():
                    raise RuntimeError("Duplicate task_name " +
                                       str(task.task_name) +
                                       " with one occurrence in " +
                                       str(task_file))
                    ...
                    if "task_name" in component_dict:
                        ...
                        if len(
                                atomic_component_keys.difference(
                                    set(component_dict.keys()))) > 0:
                            raise RuntimeError(
                                "Improperly defined component " +
                                str(component_name) + " in task " +
                                str(task.task_name) + ". Must contain keys: " +
        ...
        for task in tasks:
            ...
            for component_name, component_dict in task.components.items():
                if "task_name" in component_dict:
                    if component_dict[
                            "task_name"] not in task_name_to_task_dict:
                    ...
                    if component_dict["task_name"] in resolved_task_names:
                        task.components[component_name][
                            "task"] = copy.deepcopy(task_name_to_task_dict[
                                component_dict["task_name"]])
                        task.components[component_name][
                            "task"].task_params = component_dict["task_params"]
                    else:
        ...
        if len(unresolvable_tasks) > 0:
            error_msg = "Could not resolve the following tasks: " + "\n\t".join(
                [
                    'Subtask "' + str(dependency) + '" in task "' +
                    str(task_name) + '"'
                        ...
                        if component_dict["task_name"] in resolved_task_names:
                            task.components[component_name][
                                "task"] = copy.deepcopy(task_name_to_task_dict[
                                    component_dict["task_name"]])
                            task.components[component_name][
                                "task"].task_params = component_dict[
                                    "task_params"]
                        else:
                            resolved = False
                            break
    ...
    def __write_task_params_into_list(self, task_params_list):
        for idx, elem in enumerate(task_params_list):
            if type(elem) == str:
                ...
                task_params_list[idx] = self.__write_task_params_into_list(
                    elem)
            elif type(elem) == dict:
                task_params_list[idx] = self.__write_task_params_into_dict(
                    elem)
    ...
    def __write_task_params_into_dict(self, d):
        ...
        for key, value in dict_items:
            ...
            if type(value) == str:
                ...
                if np.char.isnumeric(
                        d[key_with_params]) and key_with_params not in [
                            "determiner"
                        ]:
    ...
    def check_component_n_instances(self,
        ...
        for obj in all_objects_cur_state:
            props_sat = self.obj_satisfies_props(obj, component["conditions"],
                                                 all_objects_cur_state)
            ...
            if not component_success:
                ...
                if (props_sat[component["primary_condition"]] or
                    ...
                    ("Sliced"
                     ...
                     == component["conditions"][
                         component["primary_condition"]].replace("Sliced", ""))
                        or
                    ("Cracked"
                     in component["conditions"][component["primary_condition"]]
                     and obj[component["primary_condition"]]
                     == component["conditions"]
                     [component["primary_condition"]].replace("Cracked", ""))):
                    # Either the primary condition is satisfied or can be with a state change
                    non_primary_prop_vals = [
                        value for (key, value) in props_sat.items()
                        if key != component["primary_condition"]
                    ]
                    if (np.all(non_primary_prop_vals)
                            and "Sliced" in component["conditions"][
                        ...
                        if allow_state_change:
                            satisifed_objects.append(obj)
                            if len(satisifed_objects
                                   ) >= num_instances_to_check:
                                component_success = True
                            continue
                    ...
                    if not component_success:
                        ...
                        if len(candidate_objects) == 0:
                            ...
                            for candidate_idx, cur_candidate in enumerate(
                                    candidate_objects):
                                if obj_candidate_dict[
                                        "num_props_sat"] > cur_candidate[
                                            "num_props_sat"] or (  # Satisifes more
                                                obj_candidate_dict[
                                                    "num_props_sat"] ==
                                                cur_candidate["num_props_sat"]
                                                and obj_candidate_dict[
                                                    "distance_to_agent"] <
                                                cur_candidate[
                                                    "distance_to_agent"]
                                            ):  # Closer
        ...
        output[
            "goal_conditions_total"] = num_conditions_per_obj * num_instances
        output["steps"] = list()
        num_problem_objects = min(len(candidate_objects),
                                  num_unsatisfied_instances_needed)
        problem_objects = candidate_objects[:num_problem_objects]
        output["goal_conditions_satisfied"] = (num_conditions_per_obj * min(
            num_instances, len(satisifed_objects))) + sum(
                [candidate["num_props_sat"] for candidate in problem_objects])
        keys_to_problem_objects = dict()
        for candidate in problem_objects:
            for key in candidate["props_sat"]:
                if not candidate["props_sat"][key] and key in component[
                        "condition_failure_descs"]:
            ...
            if key in keys_to_problem_objects:
                ...
                if representative_obj is not None:
                    output["steps"].append({
                        ...
                        True,
                        "objectId":
                        representative_obj["objectId"],
                        "objectType":
                        representative_obj["objectType"],
                        "desc":
                        desc,
                    })
        ...
        for candidate in problem_objects:
            ...
            for key in candidate["props_sat"]:
                if not candidate["props_sat"][key]:
                    output["problem_keys"][
                        candidate["object"]["objectId"]].append({
                            "objectType":
                            candidate["object"]["objectType"],
                            "determiner":
                            ...
                            component["conditions"][key],
                        })
        return output

    def check_episode_preconditions(self,
                                    simulator,
                                    all_objects_cur_state,
        ...
        for component in self.components.values():
            ...
            if "task_name" in component:
                component_success = component[
                    "task"].check_episode_preconditions(
                        simulator, all_objects_cur_state,
                ...
                component_existence_dict[
                    "condition_failure_descs"] = component[
                        "condition_failure_descs"]
                ...
                component_existence_dict["conditions"][
                    component["primary_condition"]] = component["conditions"][
                        component["primary_condition"]]
    ...
    def obj_satisfies_props(self, obj, props, all_objects):
        ...
        for prop in props:
            ...
            if prop == "objectType":
                ...
                continue
            elif prop == "simbotLastParentReceptacle" and props[
                    prop] is not None:
            ...
            elif type(props[prop]) is int and (props[prop] == 1
                                               or props[prop] == 0):
                if (obj[prop] and props[prop] == 0) or (not obj[prop]
                                                        and props[prop] == 1):
                    sats[prop] = False
                    continue
    ...
    def check_object_type(obj, desired_value):
        if obj["objectType"] == desired_value:
            return True
        elif ((obj["objectType"] == "SinkBasin" and desired_value == "Sink")
              or (obj["objectType"] == "Sink" and desired_value == "SinkBasin")
              or (obj["objectType"] == "BathtubBasin"
                  and desired_value == "Bathtub")
              or (obj["objectType"] == "Bathtub"
                  and desired_value == "BathtubBasin")):
            return True
        else:
            return False
    ...
    def check_component_all_instances(self, all_objects_cur_state, component):
        ...
        output["goal_conditions_satisfied"] = (
            num_conditions_per_obj * len(satisfied_objects)) + sum([
                sum(candidate["props_sat"].values())
                for candidate in problem_objects
            ])
        output["steps"] = list()
        keys_to_problem_objects = dict()
    ...
    def check_relation(
        ...
        for idx, head_determiner in enumerate(
                relation["head_determiner_list"]):
        ...
        tail_obj_type = self.get_obj_by_id(
            list(tail_candidate_obj_ids)[0],
            all_objects_cur_state)["objectType"]
        ...
        if tail_determiner == "a":
                ...
                if head_determiner == "all":
                    if len(cur_unsatisfied_objects) > 0:
                        ...
                        steps.append({
                            ...
                            representative_obj["objectType"],
                            "desc":
                            relation["failure_desc"],
                        })
                else:
                    num_instances_needed = num_task_instances
                    if head_determiner != "a":
                        num_instances_needed = num_task_instances * int(
                            head_determiner)
                    if len(cur_satisfied_objects) < num_instances_needed:
                        ...
                        num_unsatisfied_objects_available = min(
                            num_unsatisfied_objects_needed,
                            len(cur_unsatisfied_objects))
                        for obj in cur_unsatisfied_objects[:
            ...
            if output["success"] and len(
                    output["satisfied_objects"]) > 0 and len(
                        output["steps"]) == 0:
                representative_obj = satisfied_objects[0]
                steps.append({
                    "success": True,
            ...
            for tail_obj in tail_candidate_objects:
                ...
                for idx in range(num_head_entities):
                    ...
                    for head_obj in head_candidate_objects:
                        ...
                        if head_property_vals is not None and tail_obj_id in head_property_vals:
                            ...
                            cur_unsatisfied_objects.append(head_obj)
                            cur_unsatisfied_descs.append(
                                [relation["failure_desc"]])
                            cur_unsatisfied_keys.append([[
                                head_determiner, relation["property"],
                                tail_obj["objectType"]
                            ]])
                    ...
                    cur_tail_obj_status["per_head_status"].append({
                        ...
                        cur_satisfied_objects,
                        "unsatisfied_objects":
                        cur_unsatisfied_objects,
                        "unsatisfied_descs":
                        cur_unsatisfied_descs,
                        "unsatisfied_keys":
                        cur_unsatisfied_keys,
                        "instances_needed":
                        instances_needed,
                        "goal_conditions_satisfied":
                        min(instances_needed, len(cur_satisfied_objects)),
                        "success":
                        len(cur_satisfied_objects) >= instances_needed,
                    })
                ...
                cur_tail_obj_status[
                    "num_heads_satisfied"] = num_heads_satisfied
                cur_tail_obj_status["num_instances_satisfied"] = sum(
                    instances_per_head_satisfied)
                inserted = False
                for idx in range(len(sorted_candidates_tail_objs)):
                    if (cur_tail_obj_status["num_heads_satisfied"] >
                            sorted_candidates_tail_objs[idx]
                        ["num_heads_satisfied"]
                            or cur_tail_obj_status["num_instances_satisfied"] >
                            sorted_candidates_tail_objs[idx]
                        ["num_instances_satisfied"]):
                        sorted_candidates_tail_objs.insert(
                            idx, cur_tail_obj_status)
                        inserted = True
                        break
                if not inserted:
                ...
                if len(sorted_candidates_tail_objs
                       ) > num_tail_candidates_needed:
                    sorted_candidates_tail_objs = sorted_candidates_tail_objs[:
                                                                              num_tail_candidates_needed]

            if len(satisfied_tail_objs) == 0 and len(
                    sorted_candidates_tail_objs) == 0:
                ...
                output["success"] = len(
                    satisfied_tail_objs) >= num_task_instances
                    ...
                    for head_status in tail_obj["per_head_status"]:
                        if idx < num_task_instances:
                            output["goal_conditions_satisfied"] += head_status[
                                "goal_conditions_satisfied"]
                        output["satisfied_objects"] += head_status[
                            "satisfied_objects"]
                ...
                if num_tail_candidates_needed > 0:
                    ...
                    for tail_obj in tail_candidates:
                        for head_details in tail_obj["per_head_status"]:
                            output[
                                "goal_conditions_satisfied"] += head_details[
                                    "goal_conditions_satisfied"]
                            num_problem_instances = head_details[
                                "instances_needed"] - len(
                            ...
                            if num_problem_instances > 0:
                                ...
                                for obj in head_details[
                                    ...
                                    output["problem_keys"][
                                        obj["objectId"]].append({
                                            "determiner":
                                            head_details["head_determiner"],
                                            "property_name":
                                            relation["property"],
                                            "desired_property_value":
                                            tail_obj["tail_obj"]["objectType"],
                                            "objectType":
                                            obj["objectType"],
                                        })
                return output
        else:
            raise NotImplementedError("No support for tail determiner: " +
                                      str(tail_determiner) +
    ...
    def __get_representative_obj_id(task_output):
        if (not task_output["success"] and "candidate_objects" in task_output
        ...
        elif ("satisfied_objects" in task_output
        ...
        elif ("candidate_objects" in task_output
              and task_output["candidate_objects"] is not None
              and len(task_output["candidate_objects"]) > 0):
            return task_output["candidate_objects"][0]["objectId"]
        else:
    ...
    def check_episode_progress(self,
        ...
        for component_key, component in self.components.items():
            ...
            if component["determiner"] == "all":
                ...
                component_subgoal["problem_keys"] = component_output[
                    "problem_keys"]
                component_subgoal["goal_conditions_total"] = component_output[
                    "goal_conditions_total"]
                component_subgoal[
                    "goal_conditions_satisfied"] = component_output[
                        "goal_conditions_satisfied"]
                if not component_output["success"] or len(
                        component_output["steps"]) > 0:
                    all_subgoals.append(component_subgoal)

                per_component_satisfied_objects[
                    component_key] = component_output["satisfied_objects"]
                per_component_candidate_objects[
                    component_key] = component_output["candidate_objects"]
                if self.task_anchor_object == component_key:
                    task_satisfied_objects = component_output[
                        "satisfied_objects"]
                    task_candidate_objects = component_output[
                        "candidate_objects"]
                ...
                if "task_name" in component:
                    component_output = component[
                        "task"].check_episode_progress(
                            all_objects_cur_state,
                            simulator,
                            num_instances_needed=component_instances_needed,
                            use_task_candidates_in_relations=
                            use_task_candidates_in_relations,
                        )

                    component_subgoal = dict()
                    component_subgoal[
                        "representative_obj_id"] = self.__get_representative_obj_id(
                            component_output)
                    ...
                    if component["determiner"] not in ["a", "the", "all"]:
                        component_subgoal["description"] = (
                            component["determiner"] + " x " +
                            component_subgoal["description"])
                    component_subgoal["steps"] = self.flatten_list([[
                        step for step in subgoal["steps"]
                        if not step["success"]
                    ] for subgoal in component_output["subgoals"]])
                    component_subgoal[
                        "problem_keys"] = self.get_combined_problem_key_dict([
                            subgoal["problem_keys"]
                            for subgoal in component_output["subgoals"]
                        ])
                    component_subgoal[
                        "goal_conditions_total"] = component_output[
                            "goal_conditions_total"]
                    component_subgoal[
                        "goal_conditions_satisfied"] = component_output[
                            "goal_conditions_satisfied"]
                    ...
                    if use_task_candidates_in_relations:
                        per_component_candidate_objects[
                            component_key] = component_output[
                                "candidate_objects"]
                    if self.task_anchor_object == component_key:
                        task_satisfied_objects = component_output[
                            "satisfied_objects"]
